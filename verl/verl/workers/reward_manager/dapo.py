    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        for i in range(len(data)):
            ...
            if self.overlong_buffer_cfg is not None and self.overlong_buffer_cfg.enable:
                ...
                clamp_min = getattr(self.overlong_buffer_cfg, "clamp_min", None)
                if clamp_min is not None:
                    reward_before_clamp = reward
                    reward = max(reward, float(clamp_min))
                    if self.overlong_buffer_cfg.log:
                        reward_extra_info["reward_before_clamp"].append(reward_before_clamp)
                        reward_extra_info["reward_clamped"].append(reward != reward_before_clamp)
