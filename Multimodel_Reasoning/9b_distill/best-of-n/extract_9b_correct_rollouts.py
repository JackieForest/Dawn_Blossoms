...
DEFAULT_OUTPUT_ROOT = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/9b_distill"
)


def setup_logger(log_path: Path) -> logging.Logger:
    log_path.parent.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("extract_9b_correct_rollouts")
...
def load_verify_correct_rollout_ids(verify_path: Path) -> tuple[dict[str, set[int]], int, int]:
    correct_by_key: dict[str, set[int]] = {}
    verify_valid_rows = 0
    total_lines = 0
    if not verify_path.exists():
        return correct_by_key, verify_valid_rows, total_lines
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract 9B samples with correct_count > 0 and keep only their correct rollouts."
    )
    ...
    log_path = args.log_path or (args.output_root / "extract_9b_correct_rollouts.log")
    ...
    logger.info(
        "num_splits=%s compression=%s row_group_size=%s overwrite=%s",
        args.num_splits,
        args.compression,
        args.row_group_size,
        args.overwrite,
    ...
    mismatch_splits: list[str] = []

    for split in tqdm(range(args.num_splits), desc="extract 9B correct", dynamic_ncols=True):
        ...
        correct_ids_by_key, verify_valid_rows, verify_lines = load_verify_correct_rollout_ids(verify_path)
        ...
        if split_ids != rollout_ids:
            mismatch_splits.append(str(split))
        ...
        logger.info(
            "split=%s source_rows=%s verify_valid=%s verify_lines=%s selected_samples=%s "
            "rollout_records=%s correct_rollouts=%s split_path=%s rollout_path=%s",
            split,
            source_rows,
            verify_valid_rows,
            verify_lines,
