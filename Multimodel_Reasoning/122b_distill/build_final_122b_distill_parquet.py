...
DEFAULT_FINAL_ROLLOUTS_DIR = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/122b_distill/final_rollouts"
)
DEFAULT_OUTPUT_DIR = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/122b_distill/final"
)
DEFAULT_OUTPUT_NAME = "Final_qwen35_122b_distill-45k.parquet"
DEFAULT_LOG_FILE = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/122b_distill/build_final_122b_distill_parquet.log"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Build one final 122B distill parquet by adding the selected best rollout response "
    ...
    parser.add_argument(
        "--replace-response",
        action="store_true",
        help="Replace an existing response column in final_splits with selected final_rollouts responses.",
    )
    ...
    try:
        for split in tqdm(range(args.num_splits), desc="build final 122B parquet", dynamic_ncols=True):
            ...
            if "response" in df.columns and not args.replace_response:
