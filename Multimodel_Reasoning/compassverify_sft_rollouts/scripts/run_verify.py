...
def check_rollout_format(response: Any) -> tuple[bool, str]:
    # For SFT evaluation, judge the native full response. Do not require
    # distillation-style <think>/<answer> tags before sending to CompassVerifier.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify SFT RL rollouts with CompassVerifier.")
    ...
    with output_path.open("a", encoding="utf-8") as out_f, failed_path.open("a", encoding="utf-8") as fail_f:
        for item in tqdm(todo, desc=f"verify split {index}", dynamic_ncols=True):
            try:
                ...
                prompts: list[str] = [
                    format_prompt(tokenizer, str(item.get("question", "")), str(item.get("answer", "")), candidate)
