    ...
    for item in rollouts:
        if item.get("error"):
            return {
                **record,
                "rollout_valid": False,
                "rollouts": [r for r in rollouts if not r.get("error")],
                "error": item["error"],
                "failed_rollout_id": item.get("rollout_id", -1),
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Qwen3.5-4B thinking rollouts for one split.")
