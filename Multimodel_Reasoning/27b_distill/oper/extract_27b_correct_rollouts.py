...
DEFAULT_SOURCE_ROLLOUTS = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "qwen35_27b_distill/rollouts"
)
DEFAULT_VERIFY_DIR = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "qwen35_27b_distill/verify_response"
)
DEFAULT_OUTPUT_ROOT = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/27b_distill"
)


def setup_logger(log_path: Path) -> logging.Logger:
    log_path.parent.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("extract_27b_correct_rollouts")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract 27B samples with correct_count > 0 and keep only their correct rollouts."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Skip malformed verify_response lines. Enabled by default for known bad 27B verify JSONL lines.",
    )
    parser.add_argument(
        "--no-skip-bad-json",
        dest="skip_bad_json",
        action="store_false",
        help="Fail instead of skipping malformed verify_response lines.",
    )
    ...
    log_path = args.log_path or (args.output_root / "extract_27b_correct_rollouts.log")
    ...
    for split in tqdm(range(args.num_splits), desc="extract 27B correct", dynamic_ncols=True):
        ...
        for row in rollout_rows:
            difficulty_counts[len(row["rollouts"])] = difficulty_counts.get(len(row["rollouts"]), 0) + 1
    ...
    for correct_count in sorted(difficulty_counts):
        logger.info("correct_rollout_count=%s samples=%s", correct_count, difficulty_counts[correct_count])
