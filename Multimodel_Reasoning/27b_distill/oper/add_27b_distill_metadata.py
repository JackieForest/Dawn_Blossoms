...
DEFAULT_INPUT_SPLITS = Path(
    ...
    "distill_results/27b_distill/splits"
)
DEFAULT_INPUT_ROLLOUTS = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/27b_distill/rollouts"
)
DEFAULT_OUTPUT_SPLITS = Path(
    "/mnt/dhwfile/raise/user/linjuekai/Multimodel_Reasoning/rollouts/output/"
    "distill_results/27b_distill/final_splits"
)


def setup_logger(log_path: Path) -> logging.Logger:
    log_path.parent.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("add_27b_distill_metadata")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add distill_model and difficulty metadata to 27B correct distill splits."
    )
    ...
    parser.add_argument("--distill-model", default="qwen3.5-27b")
    ...
    log_path = args.log_path or (args.output_splits.parent / "add_27b_distill_metadata.log")
    ...
    for split in tqdm(range(args.num_splits), desc="add 27B metadata", dynamic_ncols=True):
