...
def truncate(value: Any, max_chars: int = 50000, preserve_final_answer: bool = False) -> str:
    ...
    if preserve_final_answer:
        ...
        if lines:
            final_line = lines[-1]
            if (
                re.fullmatch(r"Therefore,\s+the\s+final\s+answer\s+is\s+<answer>.+?</answer>", final_line, flags=re.DOTALL)
                and final_line not in truncated
            ):
                truncated += "\n" + final_line
    return truncated
...
def check_rollout_format(response: Any) -> tuple[bool, str]:
    ...
    if re.search(r"<\s*image\s*>", text, flags=re.IGNORECASE):
        return False, "contains_image_tag"
